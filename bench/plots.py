...
def sharpe_by_model(out_df: pd.DataFrame, path: Path) -> None:
    sharpes = {}
    for name, group in out_df.groupby("model", sort=False):
        ls = metrics.long_short_returns(group)
        sharpes[name] = metrics.portfolio_stats(ls)["sharpe"]
...
def write_all(out_df: pd.DataFrame, out_dir: Path) -> None:
    sharpe_by_model(out_df, out_dir / "sharpe_by_model.png")
